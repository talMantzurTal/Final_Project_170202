...
namespace FinalProject
{
    class PartyNode : Node
    {
        ...
        public PartyNode(Node node) : base(node)
        {
            Type t = node.GetType();
            ...
            else if (t == typeof(FinalProject.PartyNode))
            {
                PartyNode p_tmp_node = (PartyNode)node;
                m_sub_formula_tree = p_tmp_node.get_sub_formula_tree(); 
                m_children = new PartyNode [m_num_of_children];
        ...
        public Node get_sub_formula_tree()
        {
