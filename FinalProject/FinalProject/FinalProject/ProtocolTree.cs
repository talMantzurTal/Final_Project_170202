...
namespace FinalProject
{
    class ProtocolTree : Tree
    {
        ...
        {
            // Deep clone of the kw_tree (extends IDeepClonable) for reacability
            Tree egh_tree = kw_tree.deep_copy();
            ...
            foreach (Node node in kw_tree)
            {
                ...
                {
                    sub_tree_root = egh_tree.get_root(copy_flags.SHALLOW);
                }
                ...
                if (node.get_child(num_of_children_idx) == null)
                {
                    ...
                    // node.set_child(cloned_sub_tree.get_root());
                }
            }
